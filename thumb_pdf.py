...
def is_imagemagick_policy_denial(stderr_output):
...
for i, p in enumerate(pdf_files):
    ...
    try:
        stdout_output, stderr_output = pp.communicate(timeout=20)
    except TimeoutExpired:
        print("convert command did not terminate in 20 seconds, terminating.")
        pp.terminate()
        stdout_output, stderr_output = pp.communicate()
    ret = pp.returncode
    if ret is None:
        print("convert command did not terminate in 20 seconds, terminating.")
    ...
    policy_denial = stderr_output and is_imagemagick_policy_denial(stderr_output)
    if ret not in (0, None) or policy_denial:
        if policy_denial:
            print(
                "ImageMagick PDF policy blocked conversion; update policy.xml to allow PDF or use an alternate renderer.")
        else:
            print("convert failed with exit code %s. stderr: %s" % (ret, stderr_output.strip()))
    ...
    if not os.path.isfile(os.path.join(Config.tmp_dir, 'thumb-0.png')):
        ...
        cmd = "montage -mode concatenate -quality 80 -tile x1 %s %s" % (os.path.join(Config.tmp_dir, 'thumb-*.png'),
                                                                        thumb_path)
        print(cmd)
        os.system(cmd)
